...
@Component({
  ...
  styleUrl: './manager-approval.component.scss'
})
export class ManagerApprovalComponent implements OnInit {
  ...
  // Fetch submitted requests for manager 2
  loadRequests(): void {
    this.managerService.getEmployeeRequests(this.managerId).subscribe({
      next: (response: any) => {
  ...
  submitAction(): void {
    ...
    if (this.actionType === 'APPROVE') {
      this.managerService.approveRequest(reqId, 2, obj).subscribe({
        next: (res) => {
    ...
    if (this.actionType === 'REJECT') {
      this.managerService.rejectRequest(reqId, 2, obj).subscribe({
        next: (res) => {
          this.updateLocalStatusAndComments(
            reqId,
            'REJECTED',
